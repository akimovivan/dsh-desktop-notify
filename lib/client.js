  ...
  factory: (require) => {
    ...
    function makeCard(t, scope) {
      ...
      function Card(props) {
        ...
        // One skeleton for the card's three titled sections: a bordered block
        // with an uppercase title and its rows. Rows are passed as an array but
        // spread into flat children, so the rendered DOM matches the three
        // hand-written group blocks this replaced.
        var group = function (titleKey, rows) {
          return React.createElement.apply(null, [
            'div',
            { className: 'dnd-group' },
            React.createElement('div', { className: 'dnd-group-title' }, t(titleKey))
          ].concat(rows))
        }
