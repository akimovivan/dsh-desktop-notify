...
// The style is deliberate (no build step, ES5-flavoured, 2-space indent);
// see the README Notes before "modernising" it.
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS =
      ...
      '.dnd-group-title{color:var(--dsw-alias-label-secondary);padding:4px 16px;font-size:12px;font-weight:600;letter-spacing:.4px;text-transform:uppercase;display:flex;align-items:center;gap:8px;cursor:pointer;list-style:none}' +
      '.dnd-group-title::-webkit-details-marker{display:none}' +
      '.dnd-group-title:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:-2px}' +
      '.dnd-group-chevron{width:6px;height:6px;border-right:1.5px solid var(--dsw-alias-label-tertiary);border-bottom:1.5px solid var(--dsw-alias-label-tertiary);transform:rotate(-45deg);transition:transform .15s ease;flex:none}' +
      '.dnd-group[open] .dnd-group-chevron{transform:rotate(45deg)}' +
    ...
    function makeCard(t, scope) {
      ...
      function Card(props) {
        ...
        // One disclosure per titled section: a native <details> so the browser
        // owns the toggle and the rows stay mounted (hidden) while collapsed,
        // which keeps in-progress edits in the uncontrolled text fields alive.
        // No `open` prop, so every group starts collapsed on each page load.
        var group = function (titleKey, rows) {
          return React.createElement.apply(null, [
            'details',
            { className: 'dnd-group' },
            React.createElement('summary', { className: 'dnd-group-title' },
              React.createElement('span', { className: 'dnd-group-chevron', 'aria-hidden': 'true' }),
              t(titleKey)
            )
          ].concat(rows))
        }
