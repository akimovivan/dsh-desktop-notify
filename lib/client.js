...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // In-browser audition per preset: [freq, offsetSec, durSec, waveType?].
    var PREVIEW = {
    ...
    function previewSound(preset) {
      try {
        ...
        ;(PREVIEW[preset] || PREVIEW.complete).forEach(function (tone) {
          var osc = audioCtx.createOscillator()
          var gain = audioCtx.createGain()
          osc.type = tone[3] || 'sine'
          osc.frequency.setValueAtTime(tone[0], t0 + tone[1])
          gain.gain.setValueAtTime(0.0001, t0 + tone[1])
          gain.gain.exponentialRampToValueAtTime(0.2, t0 + tone[1] + 0.02)
        ...
        // Preview is best-effort; never break the card over audio.
      }
    }
    ...
    var CSS =
      ...
      '.dnd-select:disabled,.dnd-check:disabled{opacity:.5;cursor:default}' +
    ...
    function makeCard(t, scope) {
      var setField = function (field, value) {
        try { scope.set(field, value) } catch (e) { /* fenced by the scope; ignore */ }
      }
      ...
      function Card(props) {
        ...
        var fieldRow = function (field, labelKey, hintKey, on) {
          return React.createElement('div', { key: field, className: 'dnd-field' },
            React.createElement('label', { className: 'dnd-row' },
