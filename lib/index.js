 ...
 *   appName              string   default "DeepSeek Harness" — notification source name (empty = default)
...
/**
 * An empty/whitespace app name means "use the default" — mirroring soundFile's
 * empty = preset, so clearing the field in the GUI resets instead of sending
 * `notify-send -a ""`.
 */
export function resolveAppName(value) {
	return typeof value === "string" && value.trim() !== "" ? value : DEFAULTS.appName;
}
...
/**
 * Per-platform strategy table — the single place platform differences live.
 * Each entry says how that platform resolves a preset to a sound file, plays
 * a sound file (primary player plus fallback when it is missing), and shows
 * banners and alerts. Adding or changing a platform touches only this table;
 * platforms without an entry fall back to LOG_ONLY (log-only behaviour).
 */
const PLATFORMS = {
	linux: {
		soundFile: (preset) => preset.linux,
		playSound: (file) => ({
			primary: { command: "pw-play", args: [file] },
			fallback: { command: "paplay", args: [file] },
		}),
		banner: (appName, title, message) => ({ command: "notify-send", args: ["-a", appName, title, message] }),
		alert: (appName, title, message) => ({ command: "notify-send", args: ["-a", appName, "-u", "critical", title, message] }),
	},
	darwin: {
		soundFile: (preset) => `/System/Library/Sounds/${preset.mac}`,
		playSound: (file) => ({
			primary: { command: "afplay", args: [file] },
			fallback: { command: "osascript", args: ["-e", "beep"] },
		}),
		banner: (_appName, title, message) => ({ command: "osascript", args: ["-e", `display notification "${escapeAppleScript(message)}" with title "${escapeAppleScript(title)}"`] }),
		alert: (_appName, title, message) => ({ command: "osascript", args: ["-e", `display alert "${escapeAppleScript(title)}" message "${escapeAppleScript(message)}" as critical`] }),
	},
};

/** Log-only fallback for platforms without a strategy entry. */
const LOG_ONLY = {
	// Same preset resolution the old else branch gave every non-darwin platform,
	// so resolveSoundFile stays identical on all of them.
	soundFile: (preset) => preset.linux,
	playSound: () => null,
	banner: () => null,
	alert: () => null,
...
/** The one platform lookup shared by sound resolution, playback, banner, and alert. */
const platformStrategy = () => PLATFORMS[process.platform] ?? LOG_ONLY;
...
export function resolveSoundFile(opts) {
	...
	return platformStrategy().soundFile(preset);
}
...
export function createNotifier(ctx, cfg, deps = {}) {
	...
	/** Play a sound file detached, with the platform's fallback if the primary player is missing. */
	const playSound = (file) => {
		const players = platformStrategy().playSound(file);
		if (!players) return;
		try {
			const child = spawnFn(players.primary.command, players.primary.args, { stdio: "ignore", detached: true });
			child.unref();
			child.on("error", () => spawnDetached(players.fallback.command, players.fallback.args));
		} catch (error) {
	...
	const banner = (title, message) => {
		...
		const spec = platformStrategy().banner(cfg().appName, title, message);
		if (spec) spawnDetached(spec.command, spec.args);
	};
	...
	const alert = (title, message) => {
		...
		const c = cfg();
		const spec = platformStrategy().alert(c.appName, title, message);
		if (spec) spawnDetached(spec.command, spec.args);
...
export function apply(ctx, config = {}) {
	...
	const cfg = () => {
		...
		return { ...merged, appName: resolveAppName(merged.appName) };
	};
