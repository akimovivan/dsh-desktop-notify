 ...
 *   soundFile            string   default freedesktop complete.oga — Linux sound file
 *
...
function createNotifier(ctx, opts) {
	...
	/** Attention-demanding popup: sticky on Linux (swaync timeout-critical=0), modal + beep on macOS. */
	const alert = (title, message) => {
		...
		if (process.platform === "darwin") {
			...
			spawnDetached("osascript", ["-e", (opts.sound ? "beep\n" : "") + script]);
		} else if (process.platform === "linux") {
			spawnDetached("notify-send", ["-a", opts.appName, "-u", "critical", title, message]);
			if (opts.sound) spawnDetached("pw-play", [opts.soundFile]);
		}
	};
